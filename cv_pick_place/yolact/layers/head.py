...
class PredictionModule(tf.keras.layers.Layer):
    ...
    def call(self, p):
        ...
        # JAKO NEVIM ZDA TO BUDE FUNGOVAT ALE DAL JSEM TAM PROSTE napevno 1
        pred_class = tf.reshape(pred_class, [1, -1, self.num_class])
        pred_box = tf.reshape(pred_box, [1, -1, 4])
        pred_mask = tf.reshape(pred_mask, [1, -1, self.num_mask])
