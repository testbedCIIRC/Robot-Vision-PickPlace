...
def tracking_program(
    ...
    while True:
        ...
        if len(registered_packets) >= 1:
            tracked_packet = registered_packets[0]
            x, y = tracked_packet.getCentroidFromEncoder(encoder_pos)
            tracked_packet.set_centroid(x, y)
