...
class ItemsDetector:
    ...
    def __init__(
        ...
        Args:
            ...
            visual_threshold(float): Confidence threshold.
            traditional_nms(bool):
            hide_mask(bool):
            hide_bbox(bool):
            hide_score(bool):
        ...
        args = Namespace(
            weight=weights,
            img_size=img_size,
            visual_thre=visual_threshold,
            traditional_nms=traditional_nms,
            hide_mask=hide_mask,
            hide_bbox=hide_bbox,
            hide_score=hide_score,
            cutout=cutout,
            save_lincomb=save_lincomb,
            no_crop=no_crop,
            real_time=real_time,
        )
    ...
    def get_item_from_mask(
        ...
        item.set_centroid(centroid[0], centroid[1], self.homography_matrix, encoder_pos)
    ...
    def deep_item_detector(
        ...
        for i in reversed(range(num_detected)):
            if int(ids_p[i]) in self.detect_classes:
                ...
                is_cx_low_ok = (
                    item.centroid[0] - item.width / 2 < self.ignore_horizontal_px
                )
                is_cx_up_ok = item.centroid[0] + item.width / 2 > (
